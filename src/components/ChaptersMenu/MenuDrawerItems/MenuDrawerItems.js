...
const menuDrawerItems = (props) => {
    let url = props.location.pathname.slice(0,props.location.pathname.length-1)
    let chapters = null;
    if(props.chapters!==null){
        chapters = props.chapters.map(chapter=>{
            return (<MenuDrawerItem key={props.chapters.indexOf(chapter)} url={url+(parseInt(props.chapters.indexOf(chapter))+1).toString()}>{chapter}</MenuDrawerItem>)
        })
    }
